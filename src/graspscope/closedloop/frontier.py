...
failure rate and (where the domain has them) collision / intervention rates and
mean PDMS score, and detects the **safety cliff** — the coverage level below
which closed-loop safety collapses.
...
This module has no simulator dependency: it consumes the plain-JSON per-scenario
metrics produced by the closed-loop sweep scripts.
"""
...
# Closed-loop reliability metrics available in a sweep run (binary per scenario).
...
def load_scenarios(path: str | Path) -> list[dict[str, Any]]:
    """Load per-scenario metrics JSON produced by the closed-loop simulator sweep."""
