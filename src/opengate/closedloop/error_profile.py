"""Perception error profiles: how real driving perception fails.
...
- **OOV-FP** at several confidence thresholds (out-of-vocabulary detections as
  a fraction of all high-confidence B0 detections) — drives *phantoms*;
- **label confusion** probabilities (optional) — drives *mislabels*.

Profiles come from the OpenGate driving-scene gate (2D open-vocab audit on
real nuScenes frames) and the DETR3D mini-val run (3D closed-set recall).
This module is pure Python so it is CI-safe.
"""
...
class ClassProfile:
    ...
    n_gt: int = 0
    n_tp: int = 0
    # 3D-detector per-class recall (official nuScenes center-distance matching),
    # when the profile was fused from a 3D source. Kept separate from ``recall``
    # (which may come from the 2D open-vocab gate) for honest 2D/3D fusion.
    recall_3d: float | None = None
    # GraspGate: localization recall (IoU match regardless of predicted class)
    # drives "grasp the object at the wrong identity"; per-class label confusion
    # counts {predicted_class: n} for GT objects whose box was localized but
...
class PerceptionErrorProfile:
    ...
    # "2d" (open-vocab driving gate), "3d" (DETR3D official protocol), or
    # "fused" (both merged by the fusion strategy below).
...
# nuScenes detection classes in canonical order (DETR3D/FCOS3D official).
