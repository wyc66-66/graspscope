...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="opengate", description="OpenVocab-GraspGate CLI")
    parser.add_argument("--version", action="store_true", help="Print version")
    sub = parser.add_subparsers(dest="cmd")

    p_ui = sub.add_parser("ui", help="Launch the GraspGate dashboard")
...
def _cmd_ui(args: argparse.Namespace) -> int:
    ...
    from opengate.ui.app import create_app

    url = f"http://{args.host}:{args.port}/"
    print(f"OpenVocab-GraspGate UI → {url}")
    if not args.no_open:
        webbrowser.open(url)
