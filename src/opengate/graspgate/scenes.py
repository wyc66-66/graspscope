"""OpenVocab-GraspGate scene data pipeline.
...
Everything is emitted in COCO-format JSON plus an OpenGate manifest so the
existing :mod:`opengate.data` loaders and ``YoloWorldAdapter`` can consume it
...
@dataclass
class Scene:
    """One annotated scene + its OpenGate-format representation."""
...
def compose_synthetic_scene(
    ...
    *,
    ...
    def _sample_class(in_vocab: bool, used: set[str], rng: random.Random) -> str:
        pool = [c for c in (DEPLOY_VOCAB if in_vocab else OOV_PRODUCTS) if bank.available(c)]
    ...
    *,
    ...
    for si, scene in enumerate(scenes):
        ...
        for o in scene.objects:
            ...
            ann_id += 1
    coco = {
        "info": {"description": "OpenVocab-GraspGate scenes"},
        "licenses": [],
        "categories": cats,
        "images": images,
        "annotations": anns,
