"""Closed-loop grasping simulator for OpenVocab-GraspGate.
...
``run_episode`` mirrors the driving-domain closed-loop run: given a scenario
(scene + tier) it returns a binary/typed outcome, and the safety-frontier
module (:mod:`opengate.closedloop.frontier`) aggregates many episodes into the
